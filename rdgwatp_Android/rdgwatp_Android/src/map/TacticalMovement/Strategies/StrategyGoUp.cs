...
{
    class StrategyGoUp : IStrategy
    {
        //Реализация шага вверх
        public void DoAlgorithm(ref List<CreatureBuilder> Lcbs)
        {
            foreach (CreatureBuilder cb in Lcbs)
            {
                ...
                if ((cb.getType() == creatures.types.Type.PLAYER || Lcbs.Count == 1) &&
                    cb.getY() - 1 >= 0 && VisualCharacters.FloorPattern.Contains(CreatedMap.testMap[cb.getX(), cb.getY() - 1]))//CreatedMap.testMap[cb.getX(),cb.getY()] !='b' - изменится с рандомизацией карты
                {
